...
class TestRevenueCalculation:
    ...
    def test_other_costs_included_in_total(self):
        ...
        expected_other = (
            (result_no_costs["cycle_feed_cost_fcfa"] + result_no_costs["cycle_fingerlings_cost_fcfa"]) * 0.05 / 0.95
        )
        assert result_no_costs["cycle_other_costs_fcfa"] == pytest.approx(expected_other)
        assert result_no_costs["annual_other_costs_fcfa"] == pytest.approx(expected_other * 2)
