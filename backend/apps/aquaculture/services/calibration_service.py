...
class CalibrationService:
    ...
    def calibrate(
        *,
        ...
        sample_count=None,
        sample_total_weight_g=None,
        size_category="",
        notes="",
        created_offline=False,
        source_cycle_unit_allocation=None,
    ):
        existing = (
            CalibrationOperation.objects.select_related("source_cycle", "destination_cycle")
        ...
        if transferred_count <= 0 or transferred_count >= source_count:
            raise BusinessRuleViolation(
                _("Le nombre transféré doit être positif et laisser des poissons dans la source.")
            )
        ...
        destination = ProductionCycle.objects.select_for_update().filter(calibration_tank=tank, status="active").first()
        ...
        if destination is None:
            destination = ProductionCycle.objects.create(
                ...
                cycle_name=f"{tank.name} - Calibration {calibrated_at.date().isoformat()}",
                species=source.species,
                pond_identifier=tank.name,
                pond_volume_m3=tank.volume_m3,
                pond_surface_m2=None,
                infrastructure_type=["bac_calibrage"],
                start_date=calibrated_at.date(),
                initial_count=transferred_count,
                initial_average_weight=transferred_average_weight_g,
                initial_biomass=transferred_biomass,
                current_count=0,
                current_average_weight=transferred_average_weight_g,
                current_biomass=Decimal("0"),
                total_feed_consumed=Decimal("0"),
                status="active",
                unit_type="calibration",
                calibration_tank=tank,
                target_harvest_weight_g=source.target_harvest_weight_g,
                planned_harvest_date=source.planned_harvest_date,
                planned_cycle_duration_days=source.planned_cycle_duration_days,
        ...
        operation = CalibrationOperation.objects.create(
            ...
            source_cycle=source,
            source_cycle_unit_allocation=source_allocation,
            destination_cycle=destination,
            ...
            source_count_before=source_before.count,
            source_count_after=source_after.count,
            source_average_weight_before_g=source_before.average_weight_g,
            source_average_weight_after_g=source_after.average_weight_g,
            source_biomass_before_kg=source_before.biomass_kg,
            source_biomass_after_kg=source_after.biomass_kg,
            destination_count_before=destination_before.count,
            destination_count_after=destination_after.count,
        ...
        if source_allocation:
            ...
            source.current_count = sum(
                source.unit_allocations.filter(status=CycleUnitAllocation.STATUS_ACTIVE).values_list(
                    "current_fish_count", flat=True
                )
            )
            source.current_biomass = sum(
                source.unit_allocations.filter(status=CycleUnitAllocation.STATUS_ACTIVE).values_list(
                    "current_biomass_kg", flat=True
                ),
                Decimal("0"),
            )
            source.current_average_weight = (source.current_biomass * Decimal("1000") / source.current_count).quantize(
                Decimal("0.01")
            )
        else:
            source.current_count, source.current_biomass, source.current_average_weight = (
                source_after.count,
                source_after.biomass_kg,
                source_after.average_weight_g,
            )
        destination.current_count, destination.current_biomass, destination.current_average_weight = (
            destination_after.count,
            destination_after.biomass_kg,
            destination_after.average_weight_g,
        )
        ...
        warnings = []
        if (
            destination_before.count
            and abs(transferred_average_weight_g - destination_before.average_weight_g)
            / destination_before.average_weight_g
