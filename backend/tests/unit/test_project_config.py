"""
Tests unitaires pour la configuration projet mavecam_api.
"""
...
import mavecam_api
import mavecam_api.celery as celery_module
import mavecam_api.settings as project_settings
from mavecam_api.urls import api_root, health_check
...
class TestCeleryConfiguration:
    def test_package_exports_same_celery_app(self):
        assert mavecam_api.celery_app is celery_module.app

    def test_default_settings_module_constant_is_stable(self):
        assert celery_module.DEFAULT_DJANGO_SETTINGS_MODULE == 'mavecam_api.settings.development'
...
class TestSettingsModuleResolution:
    def test_resolve_settings_module_defaults_to_development(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert project_settings._resolve_settings_module() == 'mavecam_api.settings.development'

    def test_resolve_settings_module_uses_environment_value(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv('DJANGO_SETTINGS_MODULE', 'mavecam_api.settings.production')

        assert project_settings._resolve_settings_module() == 'mavecam_api.settings.production'

    def test_settings_init_selects_test_settings_when_env_requests_it(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ):
        monkeypatch.setenv('DJANGO_SETTINGS_MODULE', 'mavecam_api.settings.test')

        reloaded_settings = reload(project_settings)

        assert reloaded_settings._resolve_settings_module() == 'mavecam_api.settings.test'
