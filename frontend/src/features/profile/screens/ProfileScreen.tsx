...
export default function ProfileScreen({ navigation }: Props) {
  ...
  // Ã‰tat pour la localisation gÃ©ographique
  ...
  const handleSave = async () => {
    try {
      // Combiner les donnÃ©es de profil et de localisation
      const combinedData = {
        ...editData,
        region: locationData.region,
        department: locationData.department,
        district: locationData.arrondissement, // Mapping arrondissement vers district pour compatibilitÃ©
  ...
  return (
      <ScrollView style={styles.container}>
        ...
        {/* Informations Ferme DÃ©taillÃ©es */}
        {farmProfile && (
          <View style={styles.section}>
            ...
            <View style={styles.infoCard}>
              ...
              {farmProfile.total_area_m2 && (
                <InfoRow
                  icon="resize"
                  label={t('totalArea')}
                  value={`${farmProfile.total_area_m2} mÂ²`}
                  editable={false}
                />
              )}
...
interface InfoRowProps {
  ...
  // Nouvelles props pour les sÃ©lecteurs
  pickerOptions?: { value: string; label: string }[];
  onPickerChange?: (value: string) => void;
  // Style spÃ©cifique pour email
  isEmail?: boolean;
}
        ...
        pickerOptions && pickerOptions.length > 0 ? (
          <View style={styles.pickerWrapper}>
            <Picker
              ...
              mode="dropdown"
            >
              <Picker.Item label={placeholder || 'SÃ©lectionner...'} value="" color={MAVECAM_COLORS.GRAY_LIGHT} />
...
const styles = StyleSheet.create({
  ...
  // Styles pour le modal de zone d'intervention (identique Ã  LocationSelector)
  modalOverlay: {
    flex: 1,
  ...
  // Styles pour zone d'intervention (copiÃ©s de LocationSelector)
  selectorRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
