...
describe('features/aquaculture/services/firstCycleLaunchService', () => {
  ...
  const formData = {
    ...
    survivalRate: '95',
    productionUnits: [
      { local_id: 'unit-1', name: 'Bac 1', unit_type: 'tank', volume_m3: '25', surface_m2: '' },
    ],
  ...
  it('effectue un seul appel réseau agrégé sans persistance best effort', async () => {
    ...
    expect(mockAquaculture.launchProductionCycle).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        production_units: [
          { local_id: 'unit-1', name: 'Bac 1', unit_type: 'tank', volume_m3: 25 },
        ],
        allocations: [{ production_unit_local_id: 'unit-1', fish_count: 2100 }],
      })
    );
    const payload = mockAquaculture.launchProductionCycle.mock.calls[0][0] as any;
  ...
  it('conserve le même launch_uuid pour un retry', async () => {
    await launchFirstCycle({ formData, simulationResult, defaultPondIdentifier: 'Bassin principal' });
    await launchFirstCycle({ formData, simulationResult, defaultPondIdentifier: 'Bassin principal' });

    expect(mockAquaculture.launchProductionCycle).toHaveBeenCalledTimes(2);
    expect(mockAquaculture.launchProductionCycle.mock.calls[0][0].launch_uuid).toBe(
      formData.launchRequestId
    );
    expect(mockAquaculture.launchProductionCycle.mock.calls[1][0].launch_uuid).toBe(
      formData.launchRequestId
    );
  });
  ...
  it('bloque un lancement sans unité réelle', async () => {
    await expect(
      launchFirstCycle({
        formData: { ...formData, productionUnits: [], productionUnitAllocations: [] },
        simulationResult,
        defaultPondIdentifier: 'Bassin principal',
      })
  ...
  it('bloque une allocation partielle avant l appel réseau', async () => {
    await expect(
      launchFirstCycle({
        formData: {
          ...formData,
          productionUnitAllocations: [{ production_unit_local_id: 'unit-1', fish_count: '2000' }],
        },
        simulationResult,
        defaultPondIdentifier: 'Bassin principal',
      })
    ).rejects.toMatchObject({ translationKey: 'simulationProductionUnitAllocationInvalidError' });
    expect(mockAquaculture.launchProductionCycle).not.toHaveBeenCalled();
  });
