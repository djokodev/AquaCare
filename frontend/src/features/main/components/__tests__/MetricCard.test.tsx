...
describe("features/main/components/MetricCard", () => {
  ...
  it("renders value and label with the default props", () => {
    const { getByText } = render(
      <MetricCard value="3600" label="Total poissons" />,
    );
  ...
  it("renders value and label without scheduling timers", () => {
    const { getByText } = render(<MetricCard value="88%" label="Survie" />);
  ...
  it("renders currency and day units with a dedicated smaller text node", () => {
    const { getByText } = render(
