...
export function FormField({ label, required = false, hint, error, children }: FormFieldProps) { return <View style={styles.container}>{label ? <AppText variant="label">{label}{required ? <AppText variant="label" color="error"> *</AppText> : null}</AppText> : null}{children}{error ? <AppText variant="helper" color="error" accessibilityLiveRegion="polite">{error}</AppText> : hint ? <AppText variant="caption" color="muted">{hint}</AppText> : null}</View>; }
