...
- **Dashboard**: Multiple cards updating simultaneously
- **CyclesList**: Re-renders on filter/sort changes
- **DailyLogForm**: Input field interactions
- **ProductCatalog**: Search and filter

### Common Issues

```jsx
// Profile this pattern in AquaCare
const CycleCard = ({ cycle, onPress }) => {
  // Check if onPress causes unnecessary re-renders
  // Should use useCallback in parent
};
```
