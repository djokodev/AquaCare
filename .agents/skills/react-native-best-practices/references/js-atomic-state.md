...
AquaCare uses Redux with 5 slices: `auth`, `aquaculture`, `commerce`, `notifications`, `chat`.

### Optimization Options

1. **Keep Redux + Add Selectors**: Use `createSelector` from Reselect for memoized selectors
2. **Migrate Hot Paths to Zustand**: For frequently changing state (e.g., form inputs)
3. **Use Jotai for Local State**: Replace useState with atoms for shared component state

### Example: Cycle Filter with Zustand

```jsx
// Instead of Redux for filter state
const useCycleFilterStore = create((set) => ({
  statusFilter: 'all',
  speciesFilter: null,
...
// In component - only re-renders when statusFilter changes
const StatusFilterButton = () => {
  const statusFilter = useCycleFilterStore((s) => s.statusFilter);
  const setStatusFilter = useCycleFilterStore((s) => s.setStatusFilter);
  // ...
};
```
...
| Feature | Context | Jotai | Zustand | Redux |
|---------|---------|-------|---------|-------|
| Re-render scope | All consumers | Atom subscribers | Selector subscribers | Selector subscribers |
| Derived state | Manual | Built-in atoms | Selectors | Reselect |
| DevTools | React DevTools | Jotai DevTools | Zustand DevTools | Redux DevTools |
| Bundle size | 0 KB | ~3 KB | ~2 KB | ~7 KB |
...
- **Keep Redux**: Complex app state, time-travel debugging, existing large codebase
